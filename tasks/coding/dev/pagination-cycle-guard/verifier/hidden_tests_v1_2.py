"""Candidate-hidden acceptance tests for pagination-cycle-guard v1.2."""
...
if len(sys.argv) != 2:
    raise SystemExit("usage: hidden_tests_v1_2.py FIXTURE_ROOT")
...
class HiddenAcceptanceTest(unittest.TestCase):
    ...
    def test_cycle_back_to_nonempty_start_cursor_raises_before_refetch(self) -> None:
        source = FakeSource(
            {
                "resume": Page((Record("one", "1"),), "next"),
                "next": Page((Record("two", "2"),), "resume"),
            }
        )

        with self.assertRaises(PaginationCycleError) as caught:
            collect_records(source, start_cursor="resume", max_pages=10)

        self.assertEqual(caught.exception.cursor, "resume")
        self.assertEqual(source.calls, ["resume", "next"])
