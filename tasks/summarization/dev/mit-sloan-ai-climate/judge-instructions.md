...
4. For every key fact and key idea, assign exactly one status:
   `full`, `partial`, `missing` or `contradicted`. Provide a concise rationale
   and relevant source-location IDs.
...
- A correct external fact not present in the source is an unsupported addition,
  not bonus content.
- A reference summary is neither required nor authoritative.
- Preserve the source's epistemic status: reported observation, En-ROADS model
  result, hypothetical scenario, researcher analysis and recommendation are not
  interchangeable.
- Source-location IDs support the judgment but must not be rewarded when copied
  into a candidate response.
- Treat the complete assistant content returned by Ollama as the candidate.
  Do not silently remove leaked `<think>` reasoning or repair an answer that was
  truncated at the configured token limit; apply the corresponding hard
  constraint from the card.
- Judge content before elegance. Fluent prose cannot compensate for a reversed
  thesis or critical numeric error.
