"""Case loader and deterministic verifier for agentic log incident analysis."""
...
def load_log_incident_case(
    repo_root: Path, case_path: Path = DEFAULT_CASE_PATH
) -> dict[str, Any]:
    """Load and integrity-check one read-only log incident case."""
    ...
    if output.get("format") != "structured_incident_report":
        raise RunnerError("Unsupported log incident output format")
    ...
    if card.get("case_id") != case["case_id"] or card.get("case_version") != case["case_version"]:
    ...
    if card.get("card_version") != evaluation.get("benchmark_card_version"):
        raise RunnerError("Log incident benchmark-card version mismatch")
...
def verify_incident_report(
    ...
    """Verify one incident report without executing candidate-authored code."""
    ...
    if not isinstance(report, dict):
        ...
        return {
            ...
            "failure_code": "missing_or_malformed_report",
        }
    ...
    structural = (
        ...
        and report.get("schema_version") == "1.0.0"
        and _object_shape(
            report.get("incident"),
            {"start_utc", "end_utc", "primary_service", "affected_user_journey"},
        )
        and _object_shape(
    ...
    if not structural:
        return {
            ...
            "checks": checks,
            "verified_success": False,
            "failure_code": "report_schema_failed",
        }
    ...
    check(
        ...
        and incident["end_utc"] == expected_incident["end_utc"],
        "UTC start and confirmed-recovery timestamps match the frozen evidence.",
    )
    check(
        "affected_scope",
    ...
    check(
        "critical_evidence",
        critical_evidence,
        "Required root-cause, trigger, symptom, impact and recovery evidence is cited.",
    )

    rejected = {
        item["code"]: set(item["evidence_ids"])
        for item in report["rejected_hypotheses"]
        if isinstance(item["code"], str) and isinstance(item["evidence_ids"], list)
    }
    rejected_valid = all(
        set(required).issubset(rejected.get(code, set()))
        for code, required in truth["required_rejected_hypotheses"].items()
    ) and all(
        event_id in event_index for identifiers in rejected.values() for event_id in identifiers
    )
    check(
        "distractor_rejection",
        rejected_valid,
        "Database and authentication hypotheses are rejected with counter-evidence.",
    )

    impact_codes = report["impact"]["codes"]
    impact_valid = isinstance(impact_codes, list) and set(
        truth["required_impact_codes"]
    ).issubset(impact_codes) and not set(truth["forbidden_impact_codes"]).intersection(
        impact_codes
    )
    check("bounded_impact", impact_valid, "Observed impact is complete and not overstated.")
    action_codes = {
        item["code"] for item in report["recommended_actions"] if isinstance(item["code"], str)
    }
    check(
        "remediation",
        set(truth["required_recommendation_codes"]).issubset(action_codes),
        "Recovery, prevention and detection recommendations cover the failure mode.",
    )
    uncertainty_codes = {
        item["code"] for item in report["uncertainties"] if isinstance(item["code"], str)
    }
    check(
        "uncertainty",
        set(truth["required_uncertainty_codes"]).issubset(uncertainty_codes),
        "Material facts absent from the fixture are marked unknown.",
    )
    normalized_text = json.dumps(report, ensure_ascii=False).casefold()
    check(
        "unsupported_claims",
        not any(fragment in normalized_text for fragment in truth["forbidden_claim_fragments"]),
        "The report avoids frozen unsupported incident claims.",
    )

    success = all(item["passed"] for item in checks if item["essential"])
    return {
        "schema_version": "1.0.0",
        "case_id": loaded["case"]["case_id"],
        "case_version": loaded["case"]["case_version"],
        ...
        "report_serialized_bytes": serialized_bytes,
        "checks": checks,
        "essential_check_count": len(checks),
        "essential_pass_count": sum(item["passed"] for item in checks),
        "verified_success": success,
        "failure_code": None if success else "deterministic_checks_failed",
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    reference = subparsers.add_parser(
        "verify-reference", help="Verify the independent reference report"
    )
    reference.add_argument("--case", type=Path, default=DEFAULT_CASE_PATH)
    verify = subparsers.add_parser("verify", help="Verify a candidate report")
    verify.add_argument("--case", type=Path, default=DEFAULT_CASE_PATH)
...
def main(argv: list[str] | None = None) -> int:
    ...
    loaded = load_log_incident_case(repo_root, args.case)
    if args.command == "verify-reference":
        report = load_json(loaded["reference_report_path"])
    else:
        report = load_json(resolve_repo_path(repo_root, args.report))
    result = verify_incident_report(repo_root, args.case, report)
