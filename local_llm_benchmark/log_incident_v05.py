"""Specification-aligned verifier and response parser for log incidents v0.5."""
...
def _visible_controlled_values_valid(value: Any) -> bool:
    """Require a complete non-empty codebook that can be exposed in the task."""
    return (
        isinstance(value, dict)
        and set(value) == CONTROLLED_VALUE_FIELDS
        and all(
...
def _accepted_evidence_groups_valid(
    groups_by_hypothesis: Any, event_index: dict[str, str]
) -> bool:
    if not isinstance(groups_by_hypothesis, dict) or not groups_by_hypothesis:
        return False
    for code, groups in groups_by_hypothesis.items():
        if not isinstance(code, str) or not code or not isinstance(groups, list) or not groups:
            return False
        for group in groups:
            if (
                ...
                or not group
                or not all(
                    isinstance(event_id, str) and event_id in event_index
...
def parse_incident_report_response_v05(value: str | None) -> dict[str, Any]:
...
def load_log_incident_case_v05(repo_root: Path, case_path: Path) -> dict[str, Any]:
    """Load one additive v0.5 case and audit its visible-check mapping."""
    ...
    if (
        ...
        or case.get("case_version") != "5.0.0"
        or case.get("representation_version") != "0.5"
    ):
        raise RunnerError("Unexpected log incident v0.5 case identity")
    for key in ("case_id", "case_version", "task_path"):
        if not isinstance(case.get(key), str) or not case[key]:
            raise RunnerError(f"Log incident v0.5 case must define {key}")
    ...
    if not all(
        ...
        raise RunnerError("Log incident v0.5 case objects are incomplete")
    if (
        ...
        raise RunnerError("Log incident v0.5 context must be isolated and read-only")
    if (
        ...
        raise RunnerError("Unsupported v0.5 output contract")
    ...
    if (
        ...
        raise RunnerError("Log incident v0.5 maximum report size must be positive")
    if (
        ...
        is not True
        or visible_contract.get("version") != "0.5"
    ):
        raise RunnerError("Unexpected log incident v0.5 evaluation identity")
    ...
    if not fixture_root.is_dir():
        raise RunnerError("Log incident v0.5 fixture root does not exist")
    manifest = load_json(manifest_path)
    if (
        manifest.get("case_id") != case["case_id"]
        or manifest.get("manifest_version") != fixture.get("manifest_version")
    ):
        raise RunnerError("Log incident v0.5 fixture manifest identity mismatch")
    files = manifest.get("files")
    if not isinstance(files, list) or not files:
        raise RunnerError("Log incident v0.5 manifest must contain files")
    declared: dict[str, dict[str, Any]] = {}
    for item in files:
        if not isinstance(item, dict):
            raise RunnerError("Log incident v0.5 manifest entries must be objects")
        relative = _relative_path(item.get("path"), "v0.5 fixture manifest")
        ...
        if (
            ...
            raise RunnerError(f"Invalid v0.5 fixture declaration: {relative}")
        declared[relative] = item
    actual = _fixture_inventory(fixture_root)
    if set(actual) != set(declared):
        raise RunnerError("Log incident v0.5 fixture inventory differs from manifest")
    for relative, item in declared.items():
        expected = {"sha256": item["sha256"], "size_bytes": item["size_bytes"]}
        if actual[relative] != expected:
            raise RunnerError(f"Log incident v0.5 fixture drift: {relative}")
    visible = context.get("files")
    if not isinstance(visible, list) or visible != sorted(declared):
        raise RunnerError("Candidate-visible v0.5 inventory must be sorted and complete")
    ...
    for name, path in paths.items():
        if not path.is_file():
            raise RunnerError(f"Missing log incident v0.5 {name}: {path}")
    ...
    event_index = _event_index(fixture_root)
    if (
        truth.get("schema_version") != "4.0.0"
        or truth.get("case_id") != case["case_id"]
        or not _visible_controlled_values_valid(
            truth.get("visible_controlled_values")
        )
        or not _accepted_evidence_groups_valid(
            truth.get("accepted_rejected_hypothesis_evidence_groups"),
            event_index,
        )
    ):
        raise RunnerError("Log incident v0.5 ground-truth identity mismatch")
    if (
        ...
        raise RunnerError("Log incident v0.5 benchmark-card identity mismatch")
    if set(schema.get("required", [])) != EXPECTED_REPORT_KEYS:
        raise RunnerError("Log incident v0.5 report schema top-level keys differ")
    if normalization.get("normalization_id") != "single-intact-json-object-v0.5":
        raise RunnerError("Unexpected v0.5 response normalization identity")
    criterion_rows = criteria.get("essential_checks")
    if not isinstance(criterion_rows, list):
        raise RunnerError("Visible v0.5 criterion mapping is missing")
    if (
        criteria.get("contract_id") != "log-incident-visible-evaluation-v0.5"
        or criteria.get("supplemental_free_text_claim_review_required") is not True
        or [item.get("check_id") for item in criterion_rows] != EXPECTED_CHECK_IDS
    ):
        raise RunnerError("Visible v0.5 criterion mapping differs from verifier")
    ...
    for field, values in controlled.items():
        for value in values:
            if f"`{value}`" not in task_text:
                raise RunnerError(
                    f"Visible task omits controlled {field} value {value!r}"
                )
    expected_incident = truth["incident"]
    declared_expectations = {
        "primary_service": {expected_incident["primary_service"]},
        "affected_user_journey": {expected_incident["affected_user_journey"]},
        "root_cause_code": {truth["diagnosis"]["root_cause_code"]},
        "trigger_codes": set(truth["diagnosis"]["required_trigger_codes"]),
        "rejected_hypothesis_codes": set(
            truth["accepted_rejected_hypothesis_evidence_groups"]
        ),
        "impact_codes": set(truth["required_impact_codes"])
        | set(truth["forbidden_impact_codes"]),
        "recommendation_codes": set(truth["required_recommendation_codes"]),
        "uncertainty_codes": set(truth["required_uncertainty_codes"]),
    }
    for field, expected in declared_expectations.items():
        if not expected.issubset(controlled[field]):
            raise RunnerError(
                f"Ground truth uses a non-visible controlled {field} value"
            )
    return {
        "case": case,
...
def verify_incident_report_v05(
    ...
    check(
        ...
        "Primary service and affected journey use the correct visible canonical IDs.",
    )
    ...
    check(
        ...
        all(
            any(
                set(accepted_group).issubset(rejected.get(code, set()))
                for accepted_group in accepted_groups
            )
            for code, accepted_groups in truth[
                "accepted_rejected_hypothesis_evidence_groups"
            ].items()
        )
        and all(event_id in event_index for ids in rejected.values() for event_id in ids),
        "Each required alternative has one sufficient direct counter-evidence group.",
    )
    ...
    check(
        "controlled_code_integrity",
        incident["primary_service"] in controlled["primary_service"]
        and incident["affected_user_journey"]
        in controlled["affected_user_journey"]
        and diagnosis["root_cause_code"] in controlled["root_cause_code"]
        and set(diagnosis["trigger_codes"]).issubset(controlled["trigger_codes"])
        and set(rejected).issubset(controlled["rejected_hypothesis_codes"])
        and set(impact_codes).issubset(controlled["impact_codes"])
        and action_codes.issubset(controlled["recommendation_codes"])
        and uncertainty_codes.issubset(controlled["uncertainty_codes"]),
        "Every machine-scored categorical claim uses a visible controlled value.",
    )
    if [item["check_id"] for item in checks] != EXPECTED_CHECK_IDS:
        raise RunnerError("Log incident v0.5 verifier check order drifted")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "parse-response":
        parsed = parse_incident_report_response_v05(
    ...
    loaded = load_log_incident_case_v05(repo_root, args.case)
    report = (
    ...
    result = verify_incident_report_v05(repo_root, args.case, report)
