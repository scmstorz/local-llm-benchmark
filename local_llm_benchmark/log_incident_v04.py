"""Specification-aligned verifier and response parser for log incidents v0.4."""
...
SCOPE_GLUE_TOKENS = {"and"}


def _scope_tokens(value: Any) -> frozenset[str]:
    """Normalize descriptive scope labels without guessing domain synonyms."""
    if not isinstance(value, str):
        return frozenset()
    return frozenset(
        token
        for token in re.findall(r"[a-z0-9]+", value.casefold())
        if token not in SCOPE_GLUE_TOKENS
    )


def _scope_value_matches(expected: Any, observed: Any) -> bool:
    expected_tokens = _scope_tokens(expected)
    return bool(expected_tokens) and expected_tokens == _scope_tokens(observed)
...
def parse_incident_report_response_v04(value: str | None) -> dict[str, Any]:
...
def load_log_incident_case_v04(repo_root: Path, case_path: Path) -> dict[str, Any]:
    """Load one additive v0.4 case and audit its visible-check mapping."""
    ...
    if (
        ...
        or case.get("case_version") != "4.0.0"
        or case.get("representation_version") != "0.4"
    ):
        raise RunnerError("Unexpected log incident v0.4 case identity")
    for key in ("case_id", "case_version", "task_path"):
        if not isinstance(case.get(key), str) or not case[key]:
            raise RunnerError(f"Log incident v0.4 case must define {key}")
    ...
    if not all(
        ...
        raise RunnerError("Log incident v0.4 case objects are incomplete")
    if (
        ...
        raise RunnerError("Log incident v0.4 context must be isolated and read-only")
    if (
        ...
        raise RunnerError("Unsupported v0.4 output contract")
    ...
    if (
        ...
        raise RunnerError("Log incident v0.4 maximum report size must be positive")
    if (
        verification.get("version") != VERIFIER_VERSION
        or evaluation.get("primary_outcome") != "semantic_verified_success"
        or visible_contract.get("version") != "0.4"
    ):
        raise RunnerError("Unexpected log incident v0.4 evaluation identity")
    ...
    if not fixture_root.is_dir():
        raise RunnerError("Log incident v0.4 fixture root does not exist")
    manifest = load_json(manifest_path)
    if (
        manifest.get("case_id") != case["case_id"]
        or manifest.get("manifest_version") != fixture.get("manifest_version")
    ):
        raise RunnerError("Log incident v0.4 fixture manifest identity mismatch")
    files = manifest.get("files")
    if not isinstance(files, list) or not files:
        raise RunnerError("Log incident v0.4 manifest must contain files")
    declared: dict[str, dict[str, Any]] = {}
    for item in files:
        if not isinstance(item, dict):
            raise RunnerError("Log incident v0.4 manifest entries must be objects")
        relative = _relative_path(item.get("path"), "v0.4 fixture manifest")
        ...
        if (
            ...
            raise RunnerError(f"Invalid v0.4 fixture declaration: {relative}")
        declared[relative] = item
    actual = _fixture_inventory(fixture_root)
    if set(actual) != set(declared):
        raise RunnerError("Log incident v0.4 fixture inventory differs from manifest")
    for relative, item in declared.items():
        expected = {"sha256": item["sha256"], "size_bytes": item["size_bytes"]}
        if actual[relative] != expected:
            raise RunnerError(f"Log incident v0.4 fixture drift: {relative}")
    visible = context.get("files")
    if not isinstance(visible, list) or visible != sorted(declared):
        raise RunnerError("Candidate-visible v0.4 inventory must be sorted and complete")
    ...
    for name, path in paths.items():
        if not path.is_file():
            raise RunnerError(f"Missing log incident v0.4 {name}: {path}")
    ...
    if (
        truth.get("schema_version") != "3.0.0"
        ...
        raise RunnerError("Log incident v0.4 ground-truth identity mismatch")
    if (
        ...
        raise RunnerError("Log incident v0.4 benchmark-card identity mismatch")
    if set(schema.get("required", [])) != EXPECTED_REPORT_KEYS:
        raise RunnerError("Log incident v0.4 report schema top-level keys differ")
    if normalization.get("normalization_id") != "single-intact-json-object-v0.4":
        raise RunnerError("Unexpected v0.4 response normalization identity")
    criterion_rows = criteria.get("essential_checks")
    if not isinstance(criterion_rows, list):
        raise RunnerError("Visible v0.4 criterion mapping is missing")
    if (
        criteria.get("contract_id") != "log-incident-visible-evaluation-v0.4"
        or [item.get("check_id") for item in criterion_rows] != EXPECTED_CHECK_IDS
    ):
        raise RunnerError("Visible v0.4 criterion mapping differs from verifier")
...
def verify_incident_report_v04(
    ...
    check(
        ...
        _scope_value_matches(
            expected_incident["primary_service"], incident["primary_service"]
        )
        and _scope_value_matches(
            expected_incident["affected_user_journey"],
            incident["affected_user_journey"],
        ),
        "Primary service and affected journey match after visible label normalization.",
    )
    ...
    if [item["check_id"] for item in checks] != EXPECTED_CHECK_IDS:
        raise RunnerError("Log incident v0.4 verifier check order drifted")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "parse-response":
        parsed = parse_incident_report_response_v04(
    ...
    loaded = load_log_incident_case_v04(repo_root, args.case)
    report = (
    ...
    result = verify_incident_report_v04(repo_root, args.case, report)
