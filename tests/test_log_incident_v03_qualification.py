...
QUALIFICATIONS = [
    Path(
        ...
        "log-incident-v0.3-offline-qualification.json"
    ),
    Path("tasks/agentic/track-b/mini-log-v0.3-qualification.json"),
    Path("tasks/agentic/track-b/pi-log-v0.3-qualification.json"),
]


class LogIncidentV03QualificationTests(unittest.TestCase):
    ...
    def test_harness_qualifications_share_protocol_and_outcome_boundary(self) -> None:
        ...
        self.assertEqual(mini["protocol_id"], "agentic-log-incident-v0.3")
        self.assertEqual(pi["protocol_id"], mini["protocol_id"])
        self.assertFalse(mini["comparison_boundary"]["v02_and_v03_results_poolable"])
        self.assertFalse(pi["comparison_boundary"]["v02_and_v03_results_poolable"])
