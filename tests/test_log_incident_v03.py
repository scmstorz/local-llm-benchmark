...
from local_llm_benchmark.log_incident_v03 import (
    EXPECTED_CHECK_IDS,
...
CASES = [
    Path("tasks/agentic/log-incident-analysis/checkout-pool-regression/case-v0.3.json"),
    Path("tasks/agentic/log-incident-analysis/retry-queue-amplification/case-v0.3.json"),
    Path("tasks/agentic/log-incident-analysis/clock-skew-partial-recovery/case-v0.3.json"),
]
V02_CASE = Path(
...
class LogIncidentV03Tests(unittest.TestCase):
    def test_all_cases_are_aligned_and_references_pass_fifteen_checks(self) -> None:
        for case_path in CASES:
            with self.subTest(case=case_path):
                ...
                result = verify_incident_report_v03(REPO_ROOT, case_path, report)
                self.assertEqual(
    ...
    def test_incident_start_and_last_observation_are_independent(self) -> None:
        for case_path in CASES:
            ...
            start_result = verify_incident_report_v03(
            ...
            last_result = verify_incident_report_v03(
    ...
    def test_targeted_semantic_near_misses_fail(self) -> None:
        loaded = load_log_incident_case_v03(REPO_ROOT, CASES[0])
        reference = load_json(loaded["reference_report_path"])
        ...
        for expected_failure, mutation in mutations:
            with self.subTest(check=expected_failure):
                result = verify_incident_report_v03(REPO_ROOT, CASES[0], mutation)
    ...
    def test_schema_near_miss_fails_without_guessing(self) -> None:
        ...
        result = verify_incident_report_v03(REPO_ROOT, CASES[0], report)
        self.assertEqual(failed_ids(result), {"report_schema"})
        self.assertEqual(result["failure_code"], "report_schema_failed")

    def test_v02_representation_is_not_silently_admitted(self) -> None:
        with self.assertRaisesRegex(RunnerError, "v0.3"):
            load_log_incident_case_v03(REPO_ROOT, V02_CASE)


class LogIncidentV03ResponseParserTests(unittest.TestCase):
    def setUp(self) -> None:
        loaded = load_log_incident_case_v03(REPO_ROOT, CASES[0])
    ...
    def test_strict_object_is_valid_and_transport_compliant(self) -> None:
        result = parse_incident_report_response_v03(self.encoded)
    ...
    def test_one_fenced_object_is_recovered_but_not_strict(self) -> None:
        result = parse_incident_report_response_v03(
    ...
    def test_normalizer_never_repairs_malformed_json(self) -> None:
        result = parse_incident_report_response_v03('{"broken": }')
