...
from local_llm_benchmark.log_incident_v04 import (
    EXPECTED_CHECK_IDS,
...
CASES = [
    Path("tasks/agentic/log-incident-analysis/checkout-pool-regression/case-v0.4.json"),
    Path("tasks/agentic/log-incident-analysis/retry-queue-amplification/case-v0.4.json"),
    Path("tasks/agentic/log-incident-analysis/clock-skew-partial-recovery/case-v0.4.json"),
]
V02_CASE = Path(
...
class LogIncidentV04Tests(unittest.TestCase):
    def test_all_cases_are_aligned_and_references_pass_fifteen_checks(self) -> None:
        for case_path in CASES:
            with self.subTest(case=case_path):
                ...
                result = verify_incident_report_v04(REPO_ROOT, case_path, report)
                self.assertEqual(
    ...
    def test_incident_start_and_last_observation_are_independent(self) -> None:
        for case_path in CASES:
            ...
            start_result = verify_incident_report_v04(
            ...
            last_result = verify_incident_report_v04(
                REPO_ROOT, case_path, last_mutation
            )
            self.assertEqual(failed_ids(last_result), {"last_observed_utc"})

    def test_descriptive_scope_labels_accept_visible_equivalents(self) -> None:
        equivalents = {
            CASES[0]: ("payment api", "Checkout"),
            CASES[1]: ("order worker", "order status / order fulfillment"),
            CASES[2]: ("payment validation", "checkout authorization"),
        }
        for case_path, (service, journey) in equivalents.items():
            with self.subTest(case=case_path):
                loaded = load_log_incident_case_v04(REPO_ROOT, case_path)
                report = load_json(loaded["reference_report_path"])
    ...
    def test_scope_normalization_does_not_accept_extra_meaning(self) -> None:
        loaded = load_log_incident_case_v04(REPO_ROOT, CASES[1])
        report = load_json(loaded["reference_report_path"])
        report["incident"]["affected_user_journey"] = (
            "order status, fulfillment and checkout"
        )
        result = verify_incident_report_v04(REPO_ROOT, CASES[1], report)
    ...
    def test_partial_counterevidence_group_still_fails(self) -> None:
        loaded = load_log_incident_case_v04(REPO_ROOT, CASES[0])
        report = load_json(loaded["reference_report_path"])
        for item in report["rejected_hypotheses"]:
            if item["code"] == "authentication_failure":
                item["evidence_ids"] = ["AUTH-502"]
        result = verify_incident_report_v04(REPO_ROOT, CASES[0], report)
    ...
    def test_clock_reference_uses_each_event_under_one_direct_role(self) -> None:
        loaded = load_log_incident_case_v04(REPO_ROOT, CASES[2])
        report = load_json(loaded["reference_report_path"])
        pay_roles = [
            item["supports"]
        ...
        result = verify_incident_report_v04(REPO_ROOT, CASES[2], report)
        self.assertTrue(result["semantic_verified_success"])
    ...
    def test_targeted_semantic_near_misses_fail(self) -> None:
        ...
        unsupported_claim = copy.deepcopy(reference)
        unsupported_claim["impact"]["summary"] = "Data loss occurred."
        mutations.append(("unsupported_claims", unsupported_claim))

        for expected_failure, mutation in mutations:
            with self.subTest(check=expected_failure):
                result = verify_incident_report_v04(REPO_ROOT, CASES[0], mutation)
    ...
    def test_prior_representations_are_not_silently_admitted(self) -> None:
        for prior_case in (V02_CASE, V03_CASE):
            with self.subTest(case=prior_case):
                with self.assertRaisesRegex(RunnerError, "v0.4"):
                    load_log_incident_case_v04(REPO_ROOT, prior_case)


class LogIncidentV04ResponseParserTests(unittest.TestCase):
    def setUp(self) -> None:
        loaded = load_log_incident_case_v04(REPO_ROOT, CASES[0])
    ...
    def test_strict_object_is_valid_and_transport_compliant(self) -> None:
        result = parse_incident_report_response_v04(self.encoded)
    ...
    def test_one_fenced_object_is_recovered_but_not_strict(self) -> None:
        result = parse_incident_report_response_v04(
    ...
    def test_normalizer_never_repairs_malformed_json(self) -> None:
        result = parse_incident_report_response_v04('{"broken": }')
