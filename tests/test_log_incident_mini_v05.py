...
class MiniLogV05HarnessTests(unittest.TestCase):
    def setUp(self) -> None:
        harness = load_mini_log_v05_harness(REPO_ROOT)
    ...
    def test_harness_selects_three_unique_v05_cases(self) -> None:
        harness = load_mini_log_v05_harness(REPO_ROOT)
        ...
        self.assertTrue(
            harness["protocol"]["outcome_model"][
    ...
    def test_cross_case_read_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = step_mini_log_v05_action(REPO_ROOT, run_dir)
    ...
    def test_mocked_model_boundary_saves_one_action_without_network(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch(
                    "local_llm_benchmark.log_incident_mini_v05.OllamaClient",
                    return_value=client,
                ),
                patch(
                    "local_llm_benchmark.log_incident_mini_v05._collect_ollama_turn",
                    side_effect=fake_turn,
                ),
            ):
                result = call_mini_log_v05_model(REPO_ROOT, run_dir)
            record = load_json(run_dir / "run.json")
    ...
    def test_every_reference_can_finish_and_verify(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            for case_path in self.cases:
                with self.subTest(case=case_path):
                    run_dir = self._start(Path(temporary), case_path)
                    loaded = load_mini_log_v05_case(REPO_ROOT, case_path)
                    report = load_json(loaded["reference_report_path"])
                    self._fake_response(
                        run_dir, {"action": "finish_report", "report": report}
                    )
                    stopped = step_mini_log_v05_action(REPO_ROOT, run_dir)
                    result = finalize_mini_log_v05_run(REPO_ROOT, run_dir)
    ...
    def test_v1_case_is_not_silently_admitted(self) -> None:
        with self.assertRaisesRegex(RunnerError, "v0.5"):
            load_mini_log_v05_case(REPO_ROOT, V1_CASE)
