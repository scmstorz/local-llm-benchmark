...
from local_llm_benchmark.log_incident_confirmation_report_v05 import (
    build_log_incident_sweep_report,
)
from local_llm_benchmark.log_incident_confirmation_v05 import (
...
from local_llm_benchmark.runner import load_json, write_json
...
class LogIncidentV05ConfirmationControllerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _authorization(self, root: Path) -> Path:
        ...
        path = root / "temporary-test-authorization.json"
        write_json(
            path,
            {
                "schema_version": "1.0.0",
                "authorization_id": "log-incident-v0.5-execution-authorization",
    ...
    def test_plan_and_temporary_post_freeze_authorization_are_bound(self) -> None:
        with tempfile.TemporaryDirectory(dir=self.results) as temporary:
            ...
            self.assertTrue(
                authorization["authorization"]["live_execution_authorized"]
            )
            self.assertEqual(
                authorization["authorization"]["plan_sha256"], loaded["sha256"]
            )

    def test_freeze_records_no_repository_live_authorization(self) -> None:
        ...
        self.assertFalse(
            qualification["qualification_evidence"][
                "repository_live_authorization_present"
            ]
        )
        self.assertNotIn(
            DEFAULT_AUTHORIZATION.as_posix(),
            {item["path"] for item in qualification["artifacts"]},
        )
    ...
    def test_mocked_live_boundary_checkpoints_without_real_ollama(self) -> None:
        with tempfile.TemporaryDirectory(dir=self.results) as temporary:
            ...
            with patch(
                "local_llm_benchmark.log_incident_confirmation_v05."
