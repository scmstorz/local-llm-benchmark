...
from local_llm_benchmark.log_incident_pi_v02 import (
    PiLogV02PrepareConfig,
    build_pi_log_v02_policy,
    execute_pi_log_v02_run,
    finalize_pi_log_v02_run,
    load_pi_log_v02_case,
...
class PiLogV02HarnessTests(unittest.TestCase):
    def setUp(self) -> None:
        harness = load_pi_log_v02_harness(REPO_ROOT)
    ...
    def test_harness_pins_pi_and_three_unique_cases(self) -> None:
        loaded = load_pi_log_v02_harness(REPO_ROOT)
    ...
    def test_policy_has_time_boundary_and_no_effort_caps(self) -> None:
        loaded = load_pi_log_v02_case(REPO_ROOT, self.cases[0])
    ...
    def test_mocked_live_boundary_extracts_v2_report_without_candidate_code(self) -> None:
        with tempfile.TemporaryDirectory(dir=REPO_ROOT / "results") as temporary:
            ...
            loaded = load_pi_log_v02_case(REPO_ROOT, self.cases[2])
            ...
            with patch(
                "local_llm_benchmark.log_incident_pi_v02.subprocess.Popen",
                side_effect=selective_popen,
            ):
                result = execute_pi_log_v02_run(REPO_ROOT, run_dir)
            record = load_json(run_dir / "run.json")
    ...
    def test_every_reference_can_finalize_with_outcomes_separate(self) -> None:
        with tempfile.TemporaryDirectory(dir=REPO_ROOT / "results") as temporary:
            for case_path in self.cases:
                with self.subTest(case=case_path):
                    run_dir = self._prepare(Path(temporary), case_path)
                    loaded = load_pi_log_v02_case(REPO_ROOT, case_path)
                    report = load_json(loaded["reference_report_path"])
                    ...
                    result = finalize_pi_log_v02_run(REPO_ROOT, run_dir)
                    self.assertTrue(result["runtime_success"])
                    self.assertTrue(result["agent_termination_valid"])
    ...
    def test_v1_case_is_not_silently_admitted(self) -> None:
        with self.assertRaisesRegex(RunnerError, "v0.2"):
            load_pi_log_v02_case(REPO_ROOT, V1_CASE)
